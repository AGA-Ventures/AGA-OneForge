...
AGA OneForge's quality module turns on two things: **<Term id="nonconformance">issues</Term>** you raise against a defect and drive to closure, and **inbound inspection** that holds received goods until they're checked. Both leave a trail back to the document that went wrong.
...
<Callout type="note">
There's no dedicated supplier scorecard in AGA OneForge. **Supplier quality is a derived metric**, counted from the issues linked to each supplier. The data comes from the issues themselves, not a separate record.
</Callout>
