...
AGA OneForge's flows all end the same way: shipping, receiving, finishing a job, depreciating an asset each post to the **<Term id="general-ledger">general ledger</Term>**. This page covers the machinery they share.
...
AGA OneForge resolves the accounts to post to from a **single company-level set of defaults**. There are no per-group posting groups. The ones that recur across the flows:
