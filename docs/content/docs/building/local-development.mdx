...
description: Run the full AGA OneForge stack locally with the crbn CLI, in isolated per-worktree environments.
---

AGA OneForge is a **pnpm + Turborepo** monorepo, the ERP and MES apps plus their shared packages. Local development runs on the **`crbn` CLI** (in `packages/dev`), a per-worktree environment manager: every git worktree gets its own compose stack (Postgres, Kong, Supabase, Inngest, and Inbucket for mail) with its own ports, Redis database, and JWT credentials. So you can work several branches at once without their stacks colliding.
...
<Steps>
  ...
  This generates a `.env.local` with per-worktree ports, local Supabase keys, `REDIS_URL`, Inngest dev settings, and `DEV_BYPASS_EMAIL` so you can sign in locally without a provider redirect.
  </Step>
  <Step>
  ### Reach the apps
...
<Callout type="warn">
`DEV_BYPASS_EMAIL` is local-only. It signs you in as that email without a provider redirect. Never set it in a deployed environment. Production defaults to Google/Azure Supabase Social Login; configure provider credentials and register Supabase's `/auth/v1/callback` with the provider plus each exact app `/callback` URL in Supabase Auth before testing a real OAuth round trip.
</Callout>
...
Your local settings live in the generated `.env.local`. See [Environment variables](/docs/platform/self-hosting/environment-variables) for the full configuration surface, and [Self-hosting](/docs/platform/self-hosting) to run AGA OneForge on your own infrastructure.
