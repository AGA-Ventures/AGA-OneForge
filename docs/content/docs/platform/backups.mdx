...
It is safe to try. Before anything is replaced, AGA OneForge automatically snapshots the company's current state, so a restore is always reversible. When it finishes, the "Restored — review" card asks you to decide:
...
<FieldTable>
  <Field name="Use a demo template">Adds sample customers, suppliers, parts and quotes for the chosen industry, so you can explore AGA OneForge with realistic data.</Field>
  <Field name="Restore from a backup">Sets the new company up from an AGA OneForge backup of another company.</Field>
