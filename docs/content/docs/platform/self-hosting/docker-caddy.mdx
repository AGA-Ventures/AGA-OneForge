...
| `erp`, `mes` | The AGA OneForge apps, built from the repo. Reached at `erp.<domain>` and `mes.<domain>`. |
...
| Repo | The upstream Carbon repo checked out on the VPS (default `/opt/carbon`). |
